...
func _ready():
	...
	#$Level_Select_Menu/Button_Doom_level.connect("pressed", self, "level_select_menu_button_pressed", ["Doom_level"]) is for loading the doom level scene. 
...
#elif button_name == "Doom_level":
		#set_mouse_and_joypad_sensitivity()
		#get_node("/root/Globals" this code is for selecting the doom level from the menu.
